...
export const ContactForm: React.FC<ContactFormProps> = ({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 text-left" noValidate>
      {/* Name Field */}
      <div>
        <label htmlFor={`name-${isModal ? "modal" : "section"}`} className="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
          Full Name <span className="text-secondary">*</span>
        </label>
        <input
          id={`name-${isModal ? "modal" : "section"}`}
          type="text"
          {...register("name")}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {/* Email Field */}
        <div>
          <label htmlFor={`email-${isModal ? "modal" : "section"}`} className="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
            Email Address <span className="text-secondary">*</span>
          </label>
          <input
            id={`email-${isModal ? "modal" : "section"}`}
            type="email"
            {...register("email")}
        ...
        {/* Phone Field */}
        <div>
          <label htmlFor={`phone-${isModal ? "modal" : "section"}`} className="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
            Phone / WhatsApp <span className="text-secondary">*</span>
          </label>
          <input
            id={`phone-${isModal ? "modal" : "section"}`}
            type="tel"
            {...register("phone")}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <label htmlFor={`service-${isModal ? "modal" : "section"}`} className="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
            Service Required <span className="text-secondary">*</span>
          </label>
          <select
            id={`service-${isModal ? "modal" : "section"}`}
            {...register("service")}
            disabled={isSubmitting}
        ...
        <div>
          <label htmlFor={`scope-${isModal ? "modal" : "section"}`} className="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
            Project Scope <span className="text-slate-500 text-[11px] lowercase">(optional)</span>
          </label>
          <select
            id={`scope-${isModal ? "modal" : "section"}`}
      ...
      <div>
        <label htmlFor={`message-${isModal ? "modal" : "section"}`} className="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
          Project Details / Message <span className="text-slate-500 text-[11px] lowercase">(optional)</span>
        </label>
        <textarea
          id={`message-${isModal ? "modal" : "section"}`}
          {...register("message")}
          rows={isModal ? 3 : 4}
      ...
      {submitStatus === "success" && (
        <div className="p-3.5 rounded-xl bg-emerald-950/40 border border-emerald-800/60 text-emerald-400 text-xs sm:text-sm flex items-start gap-2.5">
      ...
      {submitStatus === "error" && (
        <div className="p-3.5 rounded-xl bg-red-950/40 border border-red-800/60 text-red-400 text-xs sm:text-sm flex items-start gap-2.5">
